...
export class MessageHandler {
  ...
  collectInputData(stepType, mode) { // stepType is simple ('request', 'observe'), mode is ('create', 'debug', 'audit')
    ...
    if (mode === 'create') {
      switch (stepType) {
        ...
        case 'codegen':
          ...
          checkboxMapping = {
            'create-codegen-appendix-frontend': 'includeFrontendAppendix'
          };
          break;
        case 'review':
          fieldMapping = {
    ...
    this.validateAndCollect(inputData, fieldMapping, checkboxMapping, requiredFields);

    logToExtension(`Collected data for ${mode}-${stepType}:`, inputData);
    return inputData;
  }

  validateAndCollect(inputData, fields, checkboxes, required) {
    ...
    // Collect all checkboxes defined in the checkbox mapping
    for (const [elementId, dataKey] of Object.entries(checkboxes)) {
      const element = document.getElementById(elementId);
      // Only include if element exists and is a checkbox
      if (element && element.type === 'checkbox') {
        inputData[dataKey] = element.checked;
      }
    }
  }
